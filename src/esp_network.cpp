...
uint32_t receiveRpcPacket(uint8_t **data, WiFiClient client)
{
    ...
    if ((header.frag & RPC_SINGLE_FRAG) && ((header.program == PORTMAP) || header.program == VXI_11_CORE))
    {
...
/* TODO: This always returns the fake Siglent ID when a DEV_READ is received. It is actually
    supposed to look at the request and return what is requested - such as C1:BSWV? expects
    the current generator config. */
void sendReadResponse(uint32_t xid, WiFiClient client)
{
    DEBUG_LN("   [network:sendReadResponse] IDN-SGLT-PRI SDG1062X");
...
#ifndef ESP32
    uint16_t timeout = 0;
    while (!client.availableForWrite() && timeout++ < 5000)
    {
        delay;
    }
#endif
...
{
    DEBUG("   [network:handlePortmap] ");

    rpcreq_getport *getport = (rpcreq_getport *)packet;

    if (getport->header.procedure != PORTMAP_GETPORT)
    {
        DEBUG_LN("ERROR: Unknown RPC packet");
    }
    else
    {
        DEBUG("PORTMAP_GETPORT - LXI_PORT=");
        DEBUG_LN(LXI_PORT);
...
void parseVxiWrite(uint8_t *packet)
{
    ...
    handleScopeMsg((char *)&(request->data), request->dataLen);
}

uint8_t handleVxi11(uint8_t *packet, WiFiClient client)
{
    DEBUG("   [network:handleVxi11] ");
...
uint8_t handlePacket(WiFiClient client)
{
    ...
    if (0 == receiveRpcPacket((uint8_t **)&header, client))
    {
        ...
        DEBUG_LN("   [network:handlePacket] ERROR: Packet Not Received");
        return 1;
    }
